...
export default function TransactionHistory() {
  ...
  return (
    <div className={theme === "dark" ? "bg-gray-900 text-gray-100 min-h-full" : "bg-gray-50 text-gray-900 min-h-full"}>
      ...
      {error && (
        <div className="p-3 mb-4 text-red-700 bg-red-200 dark:bg-red-900/30 border border-red-400 dark:border-red-700 rounded-lg">
          {error}
        </div>
      )}

      <div className={`p-5 rounded-lg shadow transition-colors ${
        ...
        <div className="flex flex-col items-center justify-between gap-2 mb-4 md:flex-row">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-white rounded-lg hover:opacity-90 transition shadow-md"
...
<div className={`relative overflow-x-auto rounded-lg transition-colors ${
  ...
  <table className={`w-full text-sm text-left transition-colors ${theme === "dark" ? "text-gray-200" : "text-gray-700"}`}>
    <thead className={`uppercase transition-colors ${
      theme === "dark" ? "bg-gray-700 text-gray-100 border-b border-gray-600" : "bg-gray-100 text-gray-700 border-b border-gray-300"
    }`}>
      <tr>
        {["Tanggal", "Deskripsi", "Tipe", "Nominal", "Aksi"].map((title, idx) => (
          <th
            key={title}
            className={`p-3 border-b cursor-pointer select-none whitespace-nowrap transition-colors ${
              theme === "dark" ? "hover:bg-gray-600 border-gray-600" : "hover:bg-gray-200 border-gray-300"
            }`}
            onClick={() => {
    ...
    <tbody className={`transition-colors ${
      ...
      {displayed.length > 0 ? (
        displayed.map((trx) => (
          <tr key={trx.id} className={`transition ${
            theme === "dark" ? "hover:bg-gray-700 border-b border-gray-600" : "hover:bg-gray-100 border-b border-gray-200"
          }`}>
